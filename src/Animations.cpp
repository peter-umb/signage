...
void Animations::runCurrentAnimation(uint8_t wait) {
    ...
    switch (currentAnimationIndex) {
        ...
        case 13:
            ...
            break;
        case 14:
            meteorShower(currentColor, 3, 4, wait);
            break;
        case 15:
            sparkle(currentColor, wait);
            break;
        case 16:
            gradientSweep(wait);
            break;
...
void Animations::fireFlicker(uint8_t wait) {
...
void Animations::wave(uint8_t wait) {
...
void Animations::randomFadeAnimation(uint8_t wait) {
  struct Color {
    uint32_t color;
    uint8_t ledStart;
    uint8_t ledCount;
  ...
  for (auto it = activeColors.begin(); it != activeColors.end(); ) {
    ...
    if (color.brightness >= 1.0) {  // Full brightness reached
      color.brightness = 1.0;
      color.fadeDirection = -1;  // Start fade-out
      color.holdDuration -= wait;
    } else if (color.brightness <= 0.0) {  // Faded out completely
...
void Animations::meteorShower(uint32_t color, uint8_t meteorSize, uint8_t trailDecay, uint8_t wait) {
...
void Animations::sparkle(uint32_t color, uint8_t wait) {
    for (int i = 0; i < 3; i++) {
...
void Animations::fireflies(uint8_t wait) {
    struct Color {
        uint32_t color;
        uint8_t led;
        float brightness;
    ...
    if (activeFireflies.size() < 10 && random(1, 10) == 1) { // Adjust probability as needed
        Color newFirefly;
...
void Animations::colorWaves(uint8_t wait) {
...
void Animations::randomSparkles(uint8_t wait) {
    struct Sparkle {
        uint8_t ledIndex;
        float brightness;
        int fadeDirection;  // 1 for fade-in, -1 for fade-out
    ...
    if (activeSparkles.size() < 10 && random(1, 20) == 1) { // Adjust probability as needed
        Sparkle newSparkle;
...
void Animations::gradientSweep(uint8_t wait) {
